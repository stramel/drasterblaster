/****************************************************************************
** Meta object code from reading C++ file 'selection.h'
**
** Created: Fri Jun 22 11:54:45 2012
**      by: The Qt Meta Object Compiler version 62 (Qt 4.7.4)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../selection.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'selection.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 62
#error "This file was generated using the moc from 4.7.4. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_Selection[] = {

 // content:
       5,       // revision
       0,       // classname
       0,    0, // classinfo
       7,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
      11,   10,   10,   10, 0x0a,
      28,   10,   10,   10, 0x0a,
      44,   10,   10,   10, 0x0a,
      52,   10,   10,   10, 0x0a,
      62,   10,   10,   10, 0x0a,
      78,   10,   10,   10, 0x08,
      91,   10,   10,   10, 0x08,

       0        // eod
};

static const char qt_meta_stringdata_Selection[] = {
    "Selection\0\0showEditAuthor()\0showUserGuide()\0"
    "about()\0aboutQt()\0showSelection()\0"
    "showWizard()\0showAdvanced()\0"
};

const QMetaObject Selection::staticMetaObject = {
    { &QMainWindow::staticMetaObject, qt_meta_stringdata_Selection,
      qt_meta_data_Selection, 0 }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &Selection::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *Selection::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *Selection::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_Selection))
        return static_cast<void*>(const_cast< Selection*>(this));
    return QMainWindow::qt_metacast(_clname);
}

int Selection::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QMainWindow::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        switch (_id) {
        case 0: showEditAuthor(); break;
        case 1: showUserGuide(); break;
        case 2: about(); break;
        case 3: aboutQt(); break;
        case 4: showSelection(); break;
        case 5: showWizard(); break;
        case 6: showAdvanced(); break;
        default: ;
        }
        _id -= 7;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
